...
const FLICKER_TL = timeline()
  .to('.candle__flame-outer', { duration: flickerSpeed, repeat: -1, yoyo: true, morphSVG: '#flame-outer' })
  .to('.candle__flame-inner', { duration: flickerSpeed, repeat: -1, yoyo: true, morphSVG: '#flame-inner' }, 0);
...
SOUNDS.TUNE.muted = SOUNDS.MATCH.muted = SOUNDS.HORN.muted =
SOUNDS.POP.muted = SOUNDS.CHEER.muted = SOUNDS.BLOW.muted =
SOUNDS.ON.muted = true;

document.querySelector('#volume').addEventListener('input', () => {
  SOUNDS.TUNE.muted = SOUNDS.MATCH.muted = SOUNDS.POP.muted =
  SOUNDS.HORN.muted = SOUNDS.CHEER.muted = SOUNDS.BLOW.muted =
  SOUNDS.ON.muted = !SOUNDS.BLOW.muted;
});

/* ---------- Custom birthday flow ---------- */
const cakeContainer = document.querySelector('.cake-container');
const birthdayMsg = document.querySelector('.birthday-message');
const nextBtn = document.getElementById('nextBtn');
const endScreen = document.querySelector('.end-screen');
const thankYouBtn = document.getElementById('thankYouBtn');
const imgFloatContainer = document.getElementById('imageFloatContainer')

MASTER_TL.addPause('FLAME_ON', () => {
  startBalloonFloat()
  birthdayMsg.classList.remove('hidden');
  imgFloatContainer.classList.remove('hidden');
...
nextBtn.addEventListener('click', () => {
  ...
  imgFloatContainer.classList.add('hidden');
  MASTER_TL.kill();
  SOUNDS.TUNE.pause();
  ...
  endScreen.classList.add('show'); // <--- makes it fade in
});

thankYouBtn.addEventListener('click', () => {
  const phoneNumber = "919869436539"; // your number
  const message = encodeURIComponent("Thank you for the birthday wish!");
...
function startBalloonFloat() {
  ...
  const minX = 10;                  // optional margin from left
    ...
    gsap.to(balloon, {
      ...
      onRepeat: function () {
        gsap.set(balloon, {
          x: gsap.utils.random(50, window.innerWidth - 50) // new random X each loop
        });
      }
    });
    ...
    // Small rotation (balloon tilt effect)
    gsap.to(balloon, {
      rotation: "+=10",
